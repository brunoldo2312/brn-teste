...
MIN_STAKE = float(os.environ.get("BRN_MIN_STAKE", "100"))  # mantido por compat
...
# ---------- Proof of Work ----------
POW_INITIAL_DIFFICULTY = int(os.environ.get("BRN_POW_DIFFICULTY", "20"))
...
class Block:
    ...
    def _registry_hash(self):
        if not self.registry_snapshot:
            return ""
        return hashlib.sha3_256(
            json.dumps(self.registry_snapshot, sort_keys=True,
                       separators=(",", ":")).encode()
        ).hexdigest()
    ...
    def verify_pow(self):
        return (self.hash == self.calculate_hash()
                and self.hash.startswith(self.target()))
    ...
    def verify(self):
        ...
        if self.miner_public_key and self.signature:
            if WalletManager.address_from_public_key(self.miner_public_key) != self.miner:
                return False
            if not WalletManager.verify_signature(
    ...
    def from_dict(cls, d):
        ...
        allowed = {"index", "timestamp", "previous_hash", "transactions",
                   "miner", "difficulty", "nonce", "miner_public_key",
                   "signature", "hash", "registry_snapshot"}
...
class Transaction:
    ...
    # Campos que participam da assinatura (canônicos)
    ...
    def sanitize(tx: dict) -> dict:
        """Remove campos que NÃO fazem parte da tx assinada (ex.: 'txid')."""
    ...
    @staticmethod
    def hash(tx):
        # hash canônico — ignora campos extras
...
class Finality:
    """Finalidade por profundidade (PoW)."""
    ...
    def is_checkpoint(self, h):
        return h > 0 and h % self.interval == 0

    def update_by_depth(self, tip_height, confirmations=6):
        new_final = max(-1, tip_height - confirmations)
        if new_final > self.finalized_height:
...
class SlashingEvidence:
    ...
    def build(invalid_block, reason, reporter_sk, reporter_pk):
        ...
        ev = SlashingEvidence(validator=invalid_block.miner, reason=reason,
...
class BlockchainDB:
    """Adaptador que expõe a interface bc.db.* esperada pelo p2p.py v2."""

    def __init__(self, blockchain):
        self.bc = blockchain

    def height(self):
        return self.bc.height

    def tip_hash(self):
        return self.bc.tip_hash
    ...
    def get_mempool_tx(self, txid):
        """Retorna a tx SEM campos extras (não quebra assinatura)."""
        for tx in self.bc.pending:
            if Transaction.hash(tx) == txid:
                return dict(tx)
        return None

    def tx_id(self, tx):
        """Hash canônico da tx — este é o txid."""
...
class Blockchain:
    ...
    def _next_difficulty(self):
        ...
        new_diff = int(chain[-1].difficulty * ratio)
        return max(POW_MIN_DIFFICULTY, new_diff)
    ...
    def _validate_tx(self, tx):
        ...
        if t == "redeem":
            a = self.registry.get_asset(asset_id)
            if not a:
                return False, "ativo inexistente"
    ...
    def add_transaction(self, tx):
        # 1) sanitiza antes de qualquer coisa
        tx = Transaction.sanitize(tx)
        with self.lock:
    ...
    def produce_block(self):
        with self.lock:
            ...
            block = Block(
                ...
                transactions=chosen,
                miner=self.node_identity["address"],
            ...
            print(f"[miner] bloco #{block.index} diff={difficulty}...")
            block.mine()
            ...
            if not block.verify():
                print("[miner] auto-verificacao falhou")
                return None
    ...
    def accept_block(self, block):
        if isinstance(block, Block):
            block = block.to_dict()
        try:
            blk = Block.from_dict(block)
        except Exception as e:
            return False, f"desserializar: {e}"

        with self.lock:
            prev = self.last_block
            if blk.hash == prev.hash:
                return True, "ja temos"
            if blk.index <= prev.index:
                return False, "altura antiga"
            if blk.previous_hash != prev.hash or blk.index != prev.index + 1:
                return False, "fora de sequencia"
            if not blk.verify():
                self._slash(blk.miner, f"PoW invalida #{blk.index}",
                            block_idx=blk.index)
            ...
            # normaliza txs
            sanitized_txs = [Transaction.sanitize(tx) for tx in blk.transactions]
            ...
            try:
                for tx in sanitized_txs:
                    if not Transaction.verify_signature(tx):
                        return False, "assinatura tx"
                    if tx["nonce"] != temp.nonce(tx["from"]):
                        return False, "nonce tx"
                    ok, why = self._validate_tx(tx)
                    if not ok:
                        return False, why
                    if not self._apply_tx_to_state(temp, tx):
                        return False, "apply tx"
                    self._apply_registry_ops(tx, persist=False)
            ...
            for tx in sanitized_txs:
                ...
                self._mempool_remove(tx)
            self.state.credit(blk.miner, NATIVE_ASSET, BLOCK_REWARD)
            hashes = {Transaction.hash(tx) for tx in sanitized_txs}
    ...
    def replace_chain(self, new_chain):
        try:
            ...
            print(f"[chain] erro desserializar: {e}")
            return False
        with self.lock:
    ...
    def faucet(self, to_address, private_key_hex, public_key_hex):
        ...
        if row and (now - row[0]) < FAUCET_COOLDOWN:
            return {"ok": False,
                    "msg": f"Aguarde {int(FAUCET_COOLDOWN-(now-row[0]))}s."}
